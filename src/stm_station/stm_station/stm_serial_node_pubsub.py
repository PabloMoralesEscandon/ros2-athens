...
class SerialPubSubNode(Node):
    def __init__(self):
        ...
        """
        super().__init__('stm_serial_node', namespace="/group_2")
        ...
        if self.serial_port.in_waiting > 0:
            try:
                ...
                if len(float_values) == 8:
                    ...
                    stm_state= STMState()
                    stm_state.motor_encoder = float_values[0]
                    stm_state.motor_velocity = float_values[1]
                    stm_state.accel_x = float_values[2]
                    stm_state.accel_y = float_values[3]
                    stm_state.accel_z = float_values[4]
                    stm_state.gyro_x = float_values[5]
                    stm_state.gyro_y = float_values[6]
                    stm_state.gyro_z = float_values[7]

                    # Publish the message
                    # TODO: Publish the float32_multi_array_msg
                    self.stm_state_publisher.publish(stm_state)
    ...
    def stm_control_callback(self, msg):
        ...
        try:
            ...
            #control_data = f"{{{1}, {500}, {0.5}, {msg.pwm}}}"
            # TODO: Encode the control data as UTF-8 and write it to the serial port 
            self.serial_port.write(control_data.encode("utf_8"))
