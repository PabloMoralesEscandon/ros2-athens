...
class STMControlNode(Node):
    def __init__(self):
        ...
        self.master_stm_state_subscriber = self.create_subscription(
            ...
            self.master_state_topic,  # <-- Replace 'NICKNAME' with the specific topic name for your group
        ...
        self.slave_stm_state_subscriber = self.create_subscription(
            STMState,             # Message type used by the publisher
            self.slave_state_topic,  # <-- Replace 'NICKNAME' with the specific topic name for your group
        ...
        self.slave_stm_control_publisher = self.create_publisher(STMControl, self.slave_control_topic, 10)  # Publisher for the slave node control
        ...
        self.control_type_service = self.create_service(STMSetControlType, 'control_type', self.service_control_type_callback)
    ...
    def service_control_type_callback(self, request, response):
        ...
        if (request.control_type == 0 or request.control_type == 1 or request.control_type==2):
